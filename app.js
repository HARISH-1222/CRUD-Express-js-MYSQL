...
exp.get('/viewdatabase', function (req, res) {
    ...
    connect.query(sql, function (err,data,fields) {
        if (err) throw err;
        res.render("../view/read.ejs",{name:data});
        // res.send(`<h1>${data[0].stdName}</h1>`);
        // res.render(__dirname+"/view/read.ejs",{userData:'data'});
        // res.send(data[0].stdName);
        // res.render('viewdatabase',{title:'User List',userData:data});
    })
});
